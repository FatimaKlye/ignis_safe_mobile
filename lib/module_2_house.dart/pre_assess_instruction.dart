    ...
    builder: (dialogContext) => AlertDialog(
      ...
      title: Row(
        children: [
          Expanded(
            child: Text(
              isTl ? 'Naka-lock ang Paunang Pagsusulit' : 'Pre-Test Locked',
...
Future<void> _openPreAssessmentIfAllowed(BuildContext context) async {
  final isTl = Localizations.localeOf(context).languageCode == 'tl';
  try {
    await ModuleProgressionService().ensureCanStartPreTest(moduleNo: 2);
    if (!context.mounted) return;
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (_) => const PreAssessmentHousePage()),
    );
  } on ProgressionAccessDenied catch (error) {
    if (!context.mounted) return;
    await _showPreAssessmentAccessDialog(
      context,
      isTl && error.message == ModuleProgressionService.preTestAlreadyTakenMessage
...
class PreAssessmentIntroPage extends StatelessWidget {
  ...
  String _moduleTitle(BuildContext context) {
    return context.tr('module_2_full_title');
  }
                      ...
                      builder: (context, constraints) {
                        ...
                        return Column(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: _AssessmentIntroCard(
                                isTl: isTl,
                                moduleTitle: _moduleTitle(context),
                              ),
                            ),
                          ],
                        );
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          isTl
                              ? 'I-tap ang gabay sa ibaba. Ipapakita lang nito ang kailangan mong tandaan bago magsimula.'
                              : 'Tap a guide below. It shows only what you need to remember before starting.',
                          softWrap: true,
                          style: TextStyle(
    ...
    switch (index) {
      case 1:
        return _GuideContent(
          ...
          body: isTl
              ? 'Basahin ang bawat tanong tungkol sa sunog sa bahay, piliin ang pinakaangkop na sagot, at isumite kapag tapos ka na.'
              : 'Read each house fire safety question, choose the best answer, and submit when you are done.',
          points: isTl
              ? <String>[
      ...
      case 2:
        return _GuideContent(
          ...
          body: isTl
              ? 'Sagutan ito bago basahin ang Module 2 para makita ang tunay mong panimulang kaalaman sa kaligtasan sa sunog sa bahay.'
              : 'Answer this before reading Module 2 so your real starting knowledge about house fire safety can be measured.',
          points: isTl
              ? <String>[
      ...
      default:
        return _GuideContent(
          ...
          body: isTl
              ? 'Ito ay maikling pagsusulit bago ang Module 2. Layunin nitong malaman kung ano na ang alam mo tungkol sa sunog sa bahay bago ka magbasa ng aralin.'
              : 'This is a short quiz before Module 2. Its purpose is to check what you already know about house fire safety before reading the lesson.',
          points: isTl
              ? <String>[
