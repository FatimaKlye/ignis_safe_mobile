...
class AppColors {
  ...
  // Module 5 mapped colors
  static const Color module5Primary = Color(0xFF7C3AED);
  static const Color module5Light = Color(0xFFEDE9FE);
  static const Color module5Soft = Color(0xFFC4B5FD);
  static const Color module5Dark = Color(0xFF5B21B6);
  static const Color module5Deep = Color(0xFF2E1065);
  static const Color module5Background = Color(0xFFF5F3FF);
  static const Color module5Card = Color(0xFFFFFFFF);
  static const Color module5Border = Color(0xFFDDD6FE);
  static const Color module5Text = Color(0xFF2E1065);
  static const Color module5MutedText = Color(0xFF6B7280);
...
class PreAssessmentBuildingPage extends StatefulWidget {
  const PreAssessmentBuildingPage({super.key});

  @override
  State<PreAssessmentBuildingPage> createState() =>
      _PreAssessmentBuildingPageState();
}

class _PreAssessmentBuildingPageState
    extends State<PreAssessmentBuildingPage>
    with AssessmentLeaveGuard<PreAssessmentBuildingPage> {
  ...
  // TEMP UNLOCK: bool _timeExpired = false;
  ...
  void _startQuizTimer() {
    ...
    _quizTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      // TEMP UNLOCK: if (_remainingSeconds <= 1) {
      //   timer.cancel();
      //   _handleTimeExpired();
      //   return;
      // }
  ...
  // TEMP UNLOCK: _handleTimeExpired commented out (timer expiry lock disabled)
  ...
  Future<void> _loadOrCreateAttempt({bool forceNewAttempt = false}) async {
    ...
    try {
      setState(() {
        _isLoading = true;
        // TEMP UNLOCK: _timeExpired = false;
  ...
  Future<void> _handleRefresh() async {
    // TEMP UNLOCK: if (_timeExpired || _isSubmitting) return;
    if (_isSubmitting) return;

    if (_showReview) {
      // TEMP UNLOCK: commented one-time retake/new-attempt lock.
  ...
  Future<void> _selectAnswer(int questionIndex, String optionId) async {
    // TEMP UNLOCK: if (_timeExpired || _isSubmitting || _showReview) return;
    if (_isSubmitting || _showReview) return;
  ...
  void _goBack() {
    ...
    // TEMP UNLOCK: if (_timeExpired) return;
  ...
  Future<void> _submitAssessment({
    ...
    // TEMP UNLOCK: bool dueToTimeUp = false,
  }) async {
    if (_isSubmitting) return;
    ...
    try {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (_) => PreAssessmentCompletionPage3(
            score: correctCount,
  ...
  // TEMP UNLOCK: _showAutoCloseInfoDialog commented out (only called by _handleTimeExpired)
  ...
  Widget _buildQuestionPager(bool compact) {
    return PageView.builder(
      ...
      // TEMP UNLOCK: physics: _timeExpired || _isSubmitting
      ...
      itemBuilder: (context, index) {
        ...
        // TEMP UNLOCK: final locked = _timeExpired || _isSubmitting;
        final locked = _isSubmitting;
          ...
          children: [
            Expanded(
              child: AssessmentSecondaryButton(
                ...
                backgroundColor: AppColors.secondaryButton,
                accentColor: AppColors.primaryButton,
                // TEMP UNLOCK: onPressed: _timeExpired || _isSubmitting
              onPressed: _isSubmitting
                    ? null
                    : () {
                        setState(() {
            ...
            Expanded(
              child: AssessmentPrimaryButton(
                ...
                // TEMP UNLOCK: onPressed: _isSubmitting || _timeExpired ? null : () => _submitAssessment(),
        ...
        children: [
          ...
          Expanded(
            child: AssessmentPrimaryButton(
              ...
              backgroundColor: AppColors.primaryButton,
              disabledBackgroundColor: AppColors.textMuted,
              // TEMP UNLOCK: onPressed: _timeExpired || _isSubmitting ? null : _goNext,
                      ...
                      children: [
                        Padding(
                          ...
                          child: _TopAssessmentBar(
                            ...
                            moduleLabel: _txt('MODULE 5', 'MODYUL 5'),
                            moduleTitle: _txt('Building Fire: Alarms, Evacuation, and Emergency Response', 'Sunog sa Gusali: Alarma, Paglikas, at Pagtugon sa Emerhensiya'),
