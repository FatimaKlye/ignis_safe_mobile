...
public class ReturnToFlutter : MonoBehaviour
{
    ...
    public void BackToFlutterApp()
    {
        ...
        try
        {
            ...
            if (activity == null)
            {
                Debug.LogError("ReturnToFlutter: currentActivity is null.");
                _isReturning = false;
                return;
            }

            Debug.Log("ReturnToFlutter: calling finish() on Unity activity.");
            ...
            activity.Call("runOnUiThread", new AndroidJavaRunnable(() =>
            {
                ...
                {
                    Debug.LogError("ReturnToFlutter finish() failed: " + e.Message);
                    _isReturning = false;
                }
                finally
                {
                    ...
                    {
                        Debug.LogError("ReturnToFlutter: System.exit(0) failed: " + ex.Message);
                    }
                }
            }));
        }
        ...
        {
            Debug.LogError("ReturnToFlutter failed: " + e.Message);
            _isReturning = false;
        }
#else
        Debug.Log("ReturnToFlutter: BackToFlutterApp called. This only returns to Flutter on Android build.");
#endif
    }
    ...
    {
        IsReturning = false;
        Debug.Log("[ReturnToFlutter] IsReturning reset (editor/testing only).");
    }
}
